...
(
select * FROM [socioec_data].[bls_su].[cpi_u_rs_1977_2017]
)
where yr=2016
;
...
proc sql;
create table hh_0 as select * from hh_base;
quit;
...
/*select d into :d from inc_target_2 where inc10=&j;*/
quit;
...
,case
/*when &d<0 and inc10_&jj = &j and i <= (&d * -1) then inc10_&jj + 1
when &d>0 and inc10_&jj = &j and i <= (&d * 1) then inc10_&jj - 1 */

when y.d<0 and x.inc10_&jj = &j and i <= (y.d * -1) then inc10_&jj + 1
when y.d>0 and inc10_&jj = &j and i <= (y.d * 1) then inc10_&jj - 1
...
inner join income_group_id as y on x.inc10_new = y.income_group_id
...
EXECUTE ( drop table if exists isam.xpef03.household_income_upgraded; ) BY ODBC ;
DISCONNECT FROM ODBC ;
quit;
*/
...
proc sql;
/*rop table sql_xpef.household_income_upgraded;*/

create table sql_xpef.household_income_upgraded(bulkload=yes bl_options=TABLOCK) as select * from hh_9;
quit;

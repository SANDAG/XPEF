...
proc import out=sd_adu_revisions
datafile="M:\RES\estimates & forecast\SR14 Forecast\Response to Aug 9th TWG meeting\City of SD\input from city on ADUs by CPA with final count.xlsx"
replace dbms=excelcs; RANGE='FINAL$a1:c56'n;
run;

proc sql noprint;
/* cap for adu in SD */
select sum(adus) into :sdadu from sd_adu_revisions;
quit;

proc sql;
CONNECT TO odbc(noprompt="driver=SQL Server; server=sql2014a8;Trusted_Connection=yes;") ;

create table prev_ac as select *
from connection to odbc
(SELECT x.*,y.cpa_id
FROM [urbansim].[urbansim].[additional_capacity] as x
inner join [isam].[xpef04].[parcel2015_mgra_jur_cpa] as y on x.parcel_id=y.parcel_id
where x.version_id = 107 and y.i=1);

create table jur_cpa_sd_1 as select *
from connection to odbc
(select parcel_id,cpa_id,jur_id from [isam].[xpef04].[parcel2015_mgra_jur_cpa] where i=1 and jur_id=14)
order by cpa_id;

disconnect from odbc;

create table jur_cpa_sd_2 as select distinct cpa_id,jur_id from jur_cpa_sd_1;
quit;

proc sql;
create table prev_ac_test as select parcel_id,count(parcel_id) as n
from prev_ac group by parcel_id having calculated n>1;
quit;


proc sql;
create table sd_adu_1 as select x.cpa_id,coalesce(y.adus,0) as adu_cap,coalesce(z.adu1,0) as adu1
,calculated adu1 - calculated adu_cap as d
from jur_cpa_sd_2 as x
left join sd_adu_revisions as y on x.cpa_id=y.cpa_id
left join (select cpa_id,sum(du) as adu1 from prev_ac where jur_id=14 and type="adu" group by cpa_id) as z
on x.cpa_id=z.cpa_id;

create table sd_adu_1a as select * from sd_adu_1 where adu_cap > adu1;
create table sd_adu_1b as select sum(d) as d from sd_adu_1 where d>0;
quit;
...
(select parcel_id,du_2018,shape.STArea() as p_area
,parcel_acres,proportion_undevelopable
,development_type_id_2018 as dev_id
,du_2018,capacity_2 as capacity,site_id,max_res_units_2018
...
proc sql;
create table p_us_1 as select x.parcel_id,x.du_2018,x.p_area,x.parcel_acres
,x.proportion_undevelopable
,x.dev_id
,coalesce(y.capacity_3,x.capacity) as capacity
,x.site_id,x.max_res_units_2018
,x.p_jur_id
from p_us_1a as x
...
from sga_jur_1) as x
inner join e1.sf1_place as y on x.jur_id=y.jur;
quit;
...
,y.du_per_acre,z.du_2018,z.remaining_cap,z.site_id
...
,case
when calculated du_sga > (z.du_2018 + z.remaining_cap)
	then calculated du_sga - z.du_2018 - z.remaining_cap
else 0
end as du_1
,ranuni(2018) as rn1
,ranuni(2019) as rn2
...
where x.du_2018=1 and x.remaining_cap=0 and x.site_id=. and x.parcel_sqft>=5000 and x.dev_id=19 and x.p_jur_id=19;
quit;
...
from adu_1a as x
inner join e1.sf1_place as y on x.p_jur_id=y.jur;
...
FROM us.urbansim_target_housing_units where version_id=111) as a
cross join 
(
...
update jur_sgoa_adu_1 set sgoa_allocated = sgoa_allocated + (adu_allocated - &sdadu) where p_jur_id = 14;
update jur_sgoa_adu_1 set adu_allocated = &sdadu where p_jur_id = 14;

update jur_sgoa_adu_1 set sgoa_allocated = sgoa_allocated - 5 where p_jur_id = 3;
update jur_sgoa_adu_1 set sgoa_allocated = sgoa_allocated - 31 where p_jur_id = 17;
update jur_sgoa_adu_1 set sgoa_allocated = sgoa_allocated + 5 + 31 where p_jur_id = 14;
...
outfile="T:\socioec\Current_Projects\XPEF06\input_data\provided and allocated capacity.xlsx"
dbms=xlsx replace;run;

proc export data=jur_sgoa_adu_3
outfile="T:\socioec\Current_Projects\XPEF06\input_data\provided and allocated capacity (rounded).xlsx"
dbms=xlsx replace;run;
*/

/* SGOA allocated should not exceed 200 for Solana Beach*/

proc sql;
create table parcels_sgoa_0 as select x.p_jur_id,x.p_id as parcel_id,x.sga_type,x.sga_name,x.du_1 as du
,y.du as du_prev
,case
when 0 < y.du < x.du_1 then y.du else x.du_1 end as du_2
from p_max_2 as x
left join (select * from prev_ac where type ^="adu") as y on x.p_id = y.parcel_id
order by p_jur_id,ranuni(2020);

create table parcel_sgoa_0a as select x.*,du_prev
from (select p_jur_id,sum(du) as du from parcels_sgoa_0 where du_prev > 0 and du <= du_prev group by p_jur_id) as x
left join (select jur_id,sum(du) as du_prev from prev_ac group by jur_id) as y
	on x.p_jur_id=y.jur_id;

/* if this table is blank, than the new version of sgoa can be accomodated with the parcels from the previous version */
create table parcel_sgoa_0b as select * from parcel_sgoa_0a where du > du_prev;
quit;


proc sql;
create table del_mar_test as select * from parcels_sgoa_0 where p_jur_id=4;
quit;


data parcels_sgoa_1(drop=du_prev);set parcels_sgoa_0(drop=du rename=(du_2=du));
where du_prev > 0;
by p_jur_id;retain i2;
...
proc sql;
create table adu_sd_1 as select x.parcel_id,x.p_jur_id,y.cpa_id
from adu_1 as x
inner join jur_cpa_sd_1 as y on x.parcel_id = y.parcel_id
where x.p_jur_id = 14
order by cpa_id,ranuni(2018);
quit;

data adu_sd_2;set adu_sd_1;by cpa_id;retain i;
if first.cpa_id then i=1;else i=i+1;
run;

proc sql;
create table adu_sd_3 as select x.*
from adu_sd_2 as x
inner join sd_adu_1 as y on x.cpa_id=y.cpa_id and x.i <= y.adu_cap;

create table adu_sd_3a as select count(*) as n from adu_sd_3;
quit;


proc sql;
create table parcels_adu_0 as select x.p_jur_id,x.parcel_id
from adu_1 as x
inner join prev_ac as y on x.parcel_id=y.parcel_id
where x.p_jur_id ^= 14 and y.type = "adu"
order by p_jur_id,ranuni(2050);
quit;
...
proc sql;
create table parcels_adu_2 as
select x.p_jur_id,x.parcel_id
...
select p_jur_id,parcel_id from adu_sd_3;

create table parcels_adu_2a as select x.*,y.adu_allocated
from (select p_jur_id,count(parcel_id) as adu from parcels_adu_2 group by p_jur_id) as x
inner join jur_sgoa_adu_2 as y on x.p_jur_id = y.p_jur_id;
quit;
...
data additional_capacity;set additional_capacity;id+1;run;

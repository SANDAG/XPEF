...
/* Updated server to ddamwsql16 from sql2014a8 on 4-Feb-2021 */
%let by0=%eval(&by1 - 1);
...
proc sql;
CONNECT TO odbc(noprompt="driver=SQL Server; server=ddamwsql16;Trusted_Connection=yes;") ;
...
proc import out=from_WU_2 datafile="T:\ABM\release_test\ABM\version_14_2_1\input\2016\mgra13_based_input2016.csv"
dbms=csv replace; run;


proc sql;
CONNECT TO odbc(noprompt="driver=SQL Server; server=sql2014a8;Trusted_Connection=yes;");
/* As of 4-Feb-2021 this was not moved to ddamwsql16 */
...
from connection to odbc
(select *
from urbansim.ref.parking_update_mgra);

disconnect from odbc;

/* All updated prices were given in 2020$, and need to be converted to 2010$ */
/* The following sharepoint link is to a document about the CPI adjustment: */
/* https://sandag.sharepoint.com/:w:/s/SANDAGModels/EeGb7kuJlDxNovuk6zAZvu0BlxB0qXWeN_ymZ725cjvd5g?e=dCE4bi 
...
*/
/* In short, divide all new price values by 1.23272 */
create table zz_final_04 as select a.*
	,b.pricehr2025/1.23272 as pricehr2025
	,b.pricehr2035/1.23272 as pricehr2035
	,b.pricehr2050/1.23272 as pricehr2050
	,b.priceday2025/1.23272 as priceday2025
	,b.priceday2035/1.23272 as priceday2035
	,b.priceday2050/1.23272 as priceday2050
	/* Monthly price needs to be converted to daily, amortized over 22 workdays */
	,(b.pricemon2025/22)/1.23272 as pricemon2025
	,(b.pricemon2035/22)/1.23272 as pricemon2035
	,(b.pricemon2050/22)/1.23272 as pricemon2050
	,b.baseline_req_pricing,b.annual_chg_2036_2050
	,b.micromobility_phase_year,b.micromobility_access_time
from zz_final_03 a
...
,CASE WHEN micromobility_phase_year IS NULL THEN MicroAccessTime
	WHEN yr >= micromobility_phase_year THEN micromobility_access_time
	ELSE MicroAccessTime
	END as MicroAccessTime 
...
proc sql;
CONNECT TO ODBC(noprompt="driver=SQL Server; server=ddamwsql16; database=isam; DBCOMMIT=10000; Trusted_Connection=yes;") ;

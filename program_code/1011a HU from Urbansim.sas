...
/* version of urbansim (usver) is set in 1010 Control Program */
...
from connection to odbc
(
select x.parcel_id,x.development_type_id_2015 as dt_2015,x.lu_2015,x.development_type_id_2017 as dt_2017,x.lu_2017
,y.gplu as lu_2099
FROM [urbansim].[urbansim].[parcel] as x
...
proc sql;
create table urb_hu_1_test as select yr,sum(du) as du
from urb_hu_1 group by yr;
quit;


proc sql;
create table urb_hu_2 as select x.*,y.dt_2017,y.lu_2017,y.lu_2099
...
proc sql;
create table urb_hu_2a as select distinct capacity_type,dt_2017,lu_2017,lu_2099,lu_name,dt_code,dt_name
...
/*
du_type assignment rules
adu -> "SFA"
(cc,mc,tc,tco,uc) -> "MF"

sch,jur : depends on lu_2099
...
proc sql;
create table urb_test_1 as select * from urb_hu_4 where mgra in (19218);
quit;

proc sql;
create table urb_test_2 as select * from p_1b where parcel_id=313396;
quit;

...
,case when age<=100 then age else 100 end as age102
,case
when hisp="H" then "H"
when r="R10" then "W"
when r="R02" then "B"
...
/*
proc sql;
create table ztest_01 as select 
*/
...
,case 
when age<=100 then age else 100 end as age102 length=3
,sex
,count(x.hp_id) as hp
...
proc sql;
create table dof_6 as select x.r7,x.age102,x.sex,x.hp_dof
...
create table dof_7 as select x.r7,x.age102,x.sex,x.size,x.hp_dof,y.hh_dof,x.hh0
...
create table dof_9 as select x.r7,x.age102,x.sex,x.size,x.i,x.hh1
,case
...
proc sort data=dof_9;by r7 age102 sex size;run;
...
/*
proc sql;
create table hh_pool_test as select x.*,y.*,x.hm_slots - y.hm_candidates as d
from (select count(*) as hh,sum(size-1) as hm_slots from hh_pool_0_orig) as x
cross join (select count(*) as hm_candidates from hm_pool_0_orig) as y;
quit;
*/
...
/* reshuffling */
proc sql;
create table test_base_1 as select x.hh_id,x.hp_id,x.role,y.size/*,z.ct,z.jur*/
,case
when x.hisp = "H" then "H"
when x.r = "R10" then "W"
when x.r = "R02" then "B"
...
,x.sex
from /*sd.hp_2010*/ hp_add_2 as x
inner join /*sd.hh_2010*/ hh_add_2 as y on x.hh_id=y.hh_id
/*inner join sd.hu_2010 as z on x.hh_id=z.hh_id*/;
...
proc sql;
create table test_bp_1 as select hh_id/*,ct,jur*/,r7,age14,sex,hp_id,age14_sex
from test_base_2 where role="M" order by /*ct,jur,*/r7,age14_sex,ranuni(&yr + 1);

create table test_bs_1 as select hh_id,r7_hh/*,ct,jur*/,age14_hh,sex_hh,age14,sex,hp_id,age14_sex
from test_base_2 where role="M" order by /*ct,jur,*/r7_hh,age14_sex,ranuni(&yr + 2);
quit;

data test_bp_2;set test_bp_1;by /*ct jur*/ r7 age14_sex;retain i;
if first.age14_sex then i=1;else i=i+1;
run;

data test_bs_2;set test_bs_1;by /*ct jur*/ r7_hh age14_sex;retain i;
if first.age14_sex then i=1;else i=i+1;
run;

proc sql;
create table test_bs_3 as select /*x.ct,x.jur,*/x.hh_id,x.hp_id as hp_id_old,x.r7_hh,x.age14_hh,x.sex_hh
,x.age14,x.sex,x.age14_sex
,y.hp_id as hp_id_new
from test_bs_2 as x
left join test_bp_2 as y on /*x.ct=y.ct and x.jur=y.jur and*/ x.r7_hh=y.r7 and x.age14_sex=y.age14_sex and x.i=y.i;
...
where y.hp_id_new = .
order by /*ct,jur,*/age14_sex,ranuni(&yr + 4);
quit;

data test_bs_3b;set test_bs_3a;by /*ct jur*/ age14_sex;retain i;
if first.age14_sex then i=1;else i=i+1;
run;

data test_bp_2b;set test_bp_2a;by /*ct jur*/ age14_sex;retain i;
if first.age14_sex then i=1;else i=i+1;
run;

proc sql;
create table test_bs_4 as select /*x.ct,x.jur,*/x.hh_id,x.hp_id_old,x.r7_hh,x.age14_hh,x.sex_hh
,x.age14,x.sex,x.age14_sex
,y.hp_id as hp_id_new
from test_bs_3b as x
left join test_bp_2b as y on /*x.ct=y.ct and x.jur=y.jur and*/ x.age14_sex=y.age14_sex and x.i=y.i;

/* slots not filled */
create table test_bs_4a as select * from test_bs_4 where hp_id_new=.
order by /*ct,jur,*/age14_sex;
...
select /*ct, jur,*/ hh_id, r7_hh, age14_hh, sex_hh, hp_id_old, hp_id_new from test_bs_3 where hp_id_new ^= .
	union all
select /*ct, jur,*/ hh_id, r7_hh, age14_hh, sex_hh, hp_id_old, hp_id_new from test_bs_4 where hp_id_new ^= .;
...
select * from hp_add_2 where role="H"
	union all
select y.hh_id, x.hp_id, x.r, x.hisp, x.sex, x.age, x.dob, x.role 
...
length hh_id 5 hp_id 5 r $3. hisp $2. sex $1. age 3 dob 4 role $1.;
run;

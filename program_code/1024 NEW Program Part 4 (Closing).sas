...
/*
proc sql;
create table hu_removed_demo_1 as select y.jur,y.ct,count(x.hh_id) as hh_rem,count(x.hu_id) as hu_rem
from hu_removed_demo as x
...
/*
proc sql;
create table dof_1a as select est_yr,sum(hh) as hh
from dof_1 group by est_yr;
quit;
*/
proc sql;
...
,hu_avl
,case
when or > 0.99 then 0.99 /* setting maximum occupancy to 99% */
when or < 0.85 and or_ct < 0.85 then 0.85 /* setting minimum occupancy to 85% */
when or < or_ct then or_ct
else or end as or format=percent7.1
,case
when vr < 0.01 then 0.01 /* setting minimum vacancy to 1% */
when vr > 0.15  and or_ct > 0.15 then 0.15 /* setting maximum vacancy to 15% */
when vr > vr_ct then vr_ct
else vr end as vr format=percent7.1
,round(hu_avl * calculated or,1) as hh0
/*
,case
when or > 0.99 then 0.99
when or < 0.01 then 0.01
else or end as or format=percent7.1
,case
when vr < 0.01 then 0.01
when vr > 0.99 then 0.99
else vr end as vr format=percent7.1
,round(hu_avl * calculated or,1) as hh0
*/
...
from (select *,hh0/sum(hh0) as s from base_vr_jurct_2 group by jur) as x
inner join hu_new_1b as y on x.jur=y.jur
order by jur,hh1,hh0;
quit;
...
/*
proc sql;
create table test_01 as select x.ct,x.jur,x.hh2,y.hu
from base_vr_jurct_4 as x
left join (select ct,jur,count(hu_id) as hu from hu_vacfil_1 group by ct,jur) as y
on x.ct=y.ct and x.jur=y.jur;

create table test_02 as select * from test_01 where hh2 > 0 and hh2 > hu;
quit;

proc sql;
create table test_03 as select * from base_vr_jurct_4 where jur=15;
quit;
*/
...
proc sql;
create table all_hh_0 as select r7/*,age14*/,count(hh_id) as hh
from
(
select x.hh_id
,case 
...
order by ct,r7/*,age14*/, ranuni(&yr + 40);

create table old_hh_1 as select ct,r7/*,age14*/,count(hh_id) as hh
from old_hh_0
group by ct, r7/*, age14*/;
...
from old_hh_2 as x
left join (select * from e1.ct_hh_target_dist where yr=&yrn) as y on x.ct=y.ct;
quit;

/*
proc sql;
create table test_01 as select * from hh_target_0 where hh=.;
quit;
*/
...
data old_hh_01;set old_hh_0;by ct r7 /*age14*/;retain i;
if first.r7/*age14*/ then i=1;else i=i+1;
run;

proc sql;
create table old_hh_02 as select x.*,z.jur
from old_hh_01 as x
inner join hh_target_2a as y on x.ct=y.ct and x.r7=y.r7/* and x.age14=y.age14*/
...
/*
proc sql;
create table test_05 as select x.*,y.hh,z.slots_orig, v.add, v.drop
from (select ct,count(*) as slots from s_7a group by ct) as x
left join (select ct,count(*) as hh from hh_target_3 group by ct) as y on x.ct=y.ct
left join (select ct,count(*) as slots_orig from s_7 group by ct) as z on x.ct=z.ct
left join (select * from hh_target_2b) as v on x.ct=v.ct
where x.slots ^= y.hh
order by slots;
quit;
*/
...
create table new_hh_1 as select r7/*, age14*/ ,size,count(hh_id) as hh
from new_hh_0 group by r7/*, age14*/ ,size;
quit;
...
data hh_target_4d;set hh_target_4c;by r7/* age14*/ size;retain i;
if first.size then i=1;else i=i+1;
run;

/* numbering households within r7/size */ 
data new_hh_2d; set new_hh_2c;by r7 /*age14*/ size;retain i;
if first.size then i=1;else i=i+1;
run;

/* assigning household to each target */
proc sql noprint;
create table hh_target_4e as select x.ct,x.r7/*,x.age14*/,x.size,x.target_id,x.slot_id,y.hh_id, "r7" as match_type length=8
from hh_target_4d as x
inner join new_hh_2d as y on x.r7=y.r7 /*and x.age14=y.age14*/ and x.size=y.size and x.i=y.i;
...
create table new_hh_2c as select hh_id,r7,size 
from new_hh_2 where slot_id = .
order by r7,size, ranuni(&c + 10);

create table new_hh_2a as select *,hh/sum(hh) as p
from (select r7,size,count(hh_id) as hh from new_hh_2 where slot_id = . group by r7, size) as x
group by r7 order by r7,ranuni(&c + 20);
quit;

data new_hh_2b;set new_hh_2a;by r7 ;retain b;
if first.r7 then do; a = 0; b = p ; end;
else if last.r7 then do; a = b; b = 1; end;
else do; a = b; b = b + p; end;
run;
...
proc sql noprint;
create table hh_target_4a as select x.ct, x.r7, x.target_id
,y.size
from (select *, ranuni(&c + 30) as rn from hh_target_4 where hh_id = .) as x
left join new_hh_2b as y on x.r7=y.r7
where y.a <= x.rn <= y.b
order by ct,size,rn;
quit;
...
proc sql noprint;
create table hh_target_4c as select x.ct,x.r7,x.size,x.target_id,y.slot_id
from hh_target_4b as x
inner join s_9b as y on x.ct=y.ct and x.size=y.size and x.i=y.i
order by r7,size,ranuni(&c + 40);
quit;
...
/*
cross join (select sum(hh3) as hh_new,sum(hp3) as hp_new,sum(hh1) as hh_tba,sum(hp1) as hp_tba,sum(hh2) as hh_drop
,sum(hh_dof) as hh_dof,sum(hp_dof) as hp_dof from jur_next_1) as z
*/
;
...
/*data sd.gq_removed_adj_&yr; set gq_rem_4;run;*/
...
/*data sd.hh_match_summary_&yr;set match_summary;run;*/ /* new */

data sd.hh_added_&yr;set /*hh_match_2*/  s_11 ;run;

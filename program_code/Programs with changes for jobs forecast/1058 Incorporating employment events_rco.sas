...
(
select
x.building_id,x.parcel_id,y.js,coalesce(u.j,0) as j,coalesce(z.mgra,0) as mgra,z.jur_&by1
FROM [urbansim].[urbansim].[building] as x
inner join (select building_id,sum(job_spaces) as js from [urbansim].[urbansim].[job_space] /*[job_space_2016]*/ group by building_id) as y
	on x.building_id=y.building_id
left join (select building_id,count(job_id) as j from [urbansim].[urbansim].[job] /*[job_2016]*/  group by building_id) as u
...
tag each building with a parcel id
overlay the centroid of each building with mgra and jur-cpa boundaries

for each parcel, select a building with the largest number of job slots
*/

proc sql;
CONNECT TO odbc(noprompt="driver=SQL Server; server=sql2014a8;Trusted_Connection=yes;") ;

create table prc_location_01 as select *
from connection to odbc
(
select a.parcel_id, a.building_id, b.js
from (select parcel_id,building_id,centroid from [urbansim].[urbansim].[building]) as a

inner join (select building_id,sum(job_spaces) as js from [urbansim].[urbansim].[job_space] group by building_id) as b
	on a.building_id=b.building_id
)
order by parcel_id,building_id,js desc;

create table prc_location_02 as select *
from connection to odbc
(
select a.parcel_id, a.building_id, coalesce(c.mgra,0) as mgra, c.jur_&by1
from [urbansim].[urbansim].[building] as a

inner JOIN [estimates].[dbo].[BLK2010_JUR_POST2010] as c on a.centroid.STIntersects(c.shape) = 1
)

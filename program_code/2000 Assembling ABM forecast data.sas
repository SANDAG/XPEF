...
if first.mgra then do;empc=0;empt=emp_total;empb=emp_total;
...
if yr=2035 then do;empc=max(empt-empb,emp_total-empb,0);empt=emp_total;empb=emp_total;
		hstallsoth2=hstallsothb+round(empc*(300/baseline_req_pricing),1);
		hstallssam2=hstallssamb+round(empc*(300/baseline_req_pricing),1);
		dstallsoth2=dstallsothb+round(empc*(300/baseline_req_pricing),1);
		dstallssam2=dstallssamb+round(empc*(300/baseline_req_pricing),1);
		mstallsoth2=mstallsothb+round(empc*(300/baseline_req_pricing),1);
		mstallssam2=mstallssamb+round(empc*(300/baseline_req_pricing),1);
		hstallsothb=hstallsoth2;hstallssamb=hstallssam2;dstallsothb=dstallsoth2;dstallssamb=dstallssam2;mstallsothb=mstallsoth2;mstallssamb=mstallssam2;
		end;
if yr>2035 then do;empc=min(empt-empb,emp_total-empb,0);empt=emp_total;empb=min(empb,emp_total);
		hstallsoth2=round(min(hstallsothb,hstallsothb+round(empc*(300/baseline_req_pricing),1))*((1+annual_chg_2036_2050)**(yr-2035)),1);
		hstallssam2=round(min(hstallssamb,hstallssamb+round(empc*(300/baseline_req_pricing),1))*((1+annual_chg_2036_2050)**(yr-2035)),1);
		dstallsoth2=round(min(dstallsothb,dstallsothb+round(empc*(300/baseline_req_pricing),1))*((1+annual_chg_2036_2050)**(yr-2035)),1);
		dstallssam2=round(min(dstallssamb,dstallssamb+round(empc*(300/baseline_req_pricing),1))*((1+annual_chg_2036_2050)**(yr-2035)),1);
		mstallsoth2=round(min(mstallsothb,mstallsothb+round(empc*(300/baseline_req_pricing),1))*((1+annual_chg_2036_2050)**(yr-2035)),1);
		mstallssam2=round(min(mstallssamb,mstallssamb+round(empc*(300/baseline_req_pricing),1))*((1+annual_chg_2036_2050)**(yr-2035)),1);
		hstallsothb=hstallsoth2;hstallssamb=hstallssam2;dstallsothb=dstallsoth2;dstallssamb=dstallssam2;mstallsothb=mstallsoth2;mstallssamb=mstallssam2;
		end;
else do;empc=max(empt-empb,emp_total-empb,0);empt=emp_total;empb=empb;
		...
		dstallssam2=dstallssamb+round(empc*(300/baseline_req_pricing),1);
		mstallsoth2=mstallsothb+round(empc*(300/baseline_req_pricing),1);
		mstallssam2=mstallssamb+round(empc*(300/baseline_req_pricing),1);end;
run;
...
quit;
/**/
/*proc sql; create table test_final_01 as */
/*select yr, sum(emp_pvt_hh) as emp_pvt_hh */
/*from final_01*/
/*group by yr*/
/*order by yr; */
...
data out_l_np_&yrr(drop=yr);set final_01_np;where yr=&yrr;run;

PROC export data=out_l_np_&yrr outfile="&abmpath\mgra13_based_input&yrr._&abmv_np..csv" DBMS=CSV REPLACE;
RUN;
...
proc sql;
create table final_01a as select * from final_01(obs=1);
quit;

proc sql;
create table final_01b as select
max(yr) as yr
, max(mgra) as mgra
, max(taz) as taz
, max(hs) as hs
...
, max(hh_mf) as hh_mf
, max(hh_mh) as hh_mh
, max(gq_civ) as gq_civ
, max(gq_mil) as gq_mil
, max(i1) as i1
, max(i2) as i2
, max(i3) as i3
, max(i4) as i4
...
, max(pop) as pop
, max(hhp) as hhp
, max(emp_ag) as emp_ag
, max(emp_const_non_bldg_prod) as emp_const_non_bldg_prod
, max(emp_const_non_bldg_office) as emp_const_non_bldg_office
, max(emp_utilities_prod) as emp_utilities_prod
, max(emp_utilities_office) as emp_utilities_office
, max(emp_const_bldg_prod) as emp_const_bldg_prod
, max(emp_const_bldg_office) as emp_const_bldg_office
, max(emp_mfg_prod) as emp_mfg_prod
, max(emp_mfg_office) as emp_mfg_office
, max(emp_whsle_whs) as emp_whsle_whs
, max(emp_trans) as emp_trans
, max(emp_retail) as emp_retail
, max(emp_prof_bus_svcs) as emp_prof_bus_svcs
, max(emp_prof_bus_svcs_bldg_maint) as emp_prof_bus_svcs_bldg_maint
, max(emp_pvt_ed_k12) as emp_pvt_ed_k12
, max(emp_pvt_ed_post_k12_oth) as emp_pvt_ed_post_k12_oth
, max(emp_health) as emp_health
, max(emp_personal_svcs_office) as emp_personal_svcs_office
, max(emp_amusement) as emp_amusement
, max(emp_hotel) as emp_hotel
, max(emp_restaurant_bar) as emp_restaurant_bar
, max(emp_personal_svcs_retail) as emp_personal_svcs_retail
, max(emp_religious) as emp_religious
, max(emp_pvt_hh) as emp_pvt_hh
, max(emp_state_local_gov_ent) as emp_state_local_gov_ent
, max(emp_fed_non_mil) as emp_fed_non_mil
, max(emp_fed_mil) as emp_fed_mil
, max(emp_state_local_gov_blue) as emp_state_local_gov_blue
, max(emp_state_local_gov_white) as emp_state_local_gov_white
, max(emp_public_ed) as emp_public_ed
, max(emp_own_occ_dwell_mgmt) as emp_own_occ_dwell_mgmt
, max(emp_fed_gov_accts) as emp_fed_gov_accts
, max(emp_st_lcl_gov_accts) as emp_st_lcl_gov_accts
, max(emp_cap_accts) as emp_cap_accts
, max(emp_total) as emp_total
, max(enrollgradekto8) as enrollgradekto8
, max(enrollgrade9to12) as enrollgrade9to12
, max(collegeenroll) as collegeenroll
, max(othercollegeenroll) as othercollegeenroll
, max(adultschenrl) as adultschenrl
, max(ech_dist) as ech_dist
, max(hch_dist) as hch_dist
, max(pseudomsa) as pseudomsa
, max(parkarea) as parkarea
, max(hstallsoth) as hstallsoth
, max(hstallssam) as hstallssam
, max(hparkcost) as hparkcost
, max(numfreehrs) as numfreehrs
, max(dstallsoth) as dstallsoth
, max(dstallssam) as dstallssam
, max(dparkcost) as dparkcost
, max(mstallsoth) as mstallsoth
, max(mstallssam) as mstallssam
, max(mparkcost) as mparkcost
,. as totint
,. as duden
,. as empden
,. as popden
...
,. as empdenbin
,. as dudenbin
/*, max(totint) as totint*/
/*, max(duden) as duden*/
/*, max(empden) as empden*/
/*, max(popden) as popden*/
/*, max(retempden) as retempden*/
/*, max(totintbin) as totintbin*/
/*, max(empdenbin) as empdenbin*/
/*, max(dudenbin) as dudenbin*/
, max(zip09) as zip09
, max(parkactive) as parkactive
, max(openspaceparkpreserve) as openspaceparkpreserve
, max(beachactive) as beachactive
, max(budgetroom) as budgetroom
, max(economyroom) as economyroom
, max(luxuryroom) as luxuryroom
, max(midpriceroom) as midpriceroom
, max(upscaleroom) as upscaleroom
, max(hotelroomtotal) as hotelroomtotal
, max(luz_id) as luz_id
, max(truckregiontype) as truckregiontype
, max(district27) as district27
, max(milestocoast) as milestocoast
, max(acres) as acres
, max(effective_acres) as effective_acres
, max(land_acres) as land_acres
, max(MicroAccessTime) as MicroAccessTime
, max(remoteAVParking) as remoteAVParking
, max(refueling_stations) as refueling_stations
from final_01;
quit;
...
EXECUTE ( drop table if exists isam.&xver..abm_mgra13_based_input; ) BY ODBC ; %PUT &SQLXRC. &SQLXMSG.;

EXECUTE
(
CREATE TABLE isam.&xver..abm_mgra13_based_input(
yr smallint
,mgra smallint
,taz smallint
,hs smallint
...
,MicroAccessTime int
,remoteAVParking int
,refueling_stations int
) WITH (DATA_COMPRESSION = PAGE)
) BY ODBC; %PUT &SQLXRC. &SQLXMSG.;


insert into sql_xpef.abm_mgra13_based_input(bulkload=yes bl_options=TABLOCK) select * from final_01;

/* Updated parking */
EXECUTE ( drop table if exists isam.&xver..abm_mgra13_based_input_np; ) BY ODBC ; %PUT &SQLXRC. &SQLXMSG.;

EXECUTE
(
CREATE TABLE isam.&xver..abm_mgra13_based_input_np(
yr smallint
,mgra smallint
,taz smallint
,hs smallint
...
insert into sql_xpef.abm_mgra13_based_input_np(bulkload=yes bl_options=TABLOCK) select * from final_01_np;
quit;

...
proc sql;
create table final_01 as select x.*,x1.*,x2.*,x3.*,x4.*,x5.*,x6.*
...
proc sql;
create table zz_final_01 as select x.*,x1.*,x2.*,x3.*,x4.*,x5.*,x6.*
from ready_part_1 as x
...
left join (select * from part_3b where yr in (2016,2018,2020,2025,2030,2035,2040,2045,2050)) as x5 on x.yr=x5.yr and x.mgra=x5.mgra

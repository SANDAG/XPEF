...
proc sql;
create table cest_1 as select r7 length=1,age102,sex,p_cest
from e1.dof_pop_proj_r7_age102 where yr=&yrn;
quit;

proc sql;
create table hp_cest_1 as select r7 length=1, age as age102,sex,hp as hp_cest
...
create table gq_next_1a as select age102,sex,r7,count(gq_id) as gq
from gq_next_1 group by age102,sex,r7;
quit;


proc sql noprint;
create table cest_2 as select x.*
,coalesce(y.gq,0) as qg
,x.p_cest - coalesce(y.gq,0) as hp_cest
from cest_1 as x left join gq_next_1a as y
on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex;

create table cest_2a as select * from cest_2 where hp_cest<0;
/* create table cest_2b as select * from cest_2 where hp_cest<20;*/

select count(*) into :err1 from cest_2a;
quit;

%macro err1;
%if &err1 >0 %then %do;
%put ERROR with cest_2a for year &yr;
%end;
%mend err1;
...
proc sql;
create table gq_rem_1 as select x.*
from gq_next_1 as x
inner join cest_2a as y on x.age102=y.age102 and x.sex=y.sex and x.r7=y.r7
order by age102,sex,r7,ranuni(&yr+11);
quit;

data gq_rem_2;set gq_rem_1;by age102 sex r7;retain i;length i 4;
if first.r7 then i=1;else i=i+1;
run;

/* these should be removed; and similar combinations of age102-sex-r7 should not be added */
proc sql;
create table gq_rem_3 as select x.*
from gq_rem_2 as x
inner join cest_2a as y on x.age102=y.age102 and x.sex=y.sex and x.r7=y.r7
where x.i <= (y.hp_cest*-1);

create table gq_rem_4 as select jur,r,hisp,age,dob,gq_type,gq_id from gq_rem_3;
quit;

proc sql;
create table gq_next_2 as select x.*
from gq_next_1 as x
left join gq_rem_3 as y on x.gq_id=y.gq_id
where y.gq_id=. 
order by jur,ranuni(&yr+12);
quit;

/*
proc sql;
create table gq_change as select x.yr, x.jur, x.gq_dof, coalesce(y.gq,0) as gq_base, x.gq_dof - coalesce(y.gq,0) as gq_d
from (select est_yr as yr,jur,gq as gq_dof from dof_1 where est_yr = &yrn) as x
left join (select jur,count(gq_id) as gq from gq_next_2 group by jur) as y
on x.jur=y.jur;
quit;

proc sql;
create table gq_add_0 as select x.*
from gq_next_2 as x
left join cest_2b as y on x.age102=y.age102 and x.sex=y.sex and x.r7=y.r7
where y.age102=.
order by jur,ranuni(&yr+13);
quit;

data gq_add_1;set gq_add_0;by jur;retain i;length i 4;
if first.jur then i=1;else i=i+1;
run;

data gq_next_2;set gq_next_2;by jur;retain i;length i 4;
if first.jur then i=1;else i=i+1;
run;

proc sql;
create table gq_add as select x.jur,x.r,x.hisp,x.sex,x.age,x.dob,x.gq_type,monotonic() as gq_id length=4
from gq_add_1 as x
inner join (select * from gq_change where gq_d > 0) as y
	on x.jur=y.jur where x.i <= y.gq_d;

update gq_add set gq_id = gq_id + (select max(gq_id) from gq_base);

create table gq_remove as select x.jur,x.r,x.hisp,x.sex,x.age,x.dob,x.gq_type,x.gq_id
from gq_next_2 as x
inner join (select * from gq_change where gq_d < 0) as y
on x.jur=y.jur where x.i <= y.gq_d *-1;

create table gq_keep as select x.jur,x.r,x.hisp,x.sex,x.age,x.dob,x.gq_type,x.gq_id
from gq_next_2 as x
left join gq_remove as y on x.gq_id = y.gq_id
where y.gq_id = .;
quit;

data gq_next_3;set gq_keep gq_add;run;
*/
...
data gq_add_1(drop=yr gq);set e1.gq_dev;where yr=&yr;
do i=1 to gq;
	output;
end;
run;
...
,y.age,y.r,y.hisp,y.sex,y.dob

from gq_add_1 as x
inner join gq_next_2b as y on x.gq_type=y.gq_type and x.j=y.j;
quit;

data gq_next_3;set gq_next_2(drop=r7 age102) gq_add_2;run;
...
data gq_next_3;set gq_next_2(drop=r7 age102);run;
...
/*
proc sql;
create table sandag_gq_2 as select 
case 
when hisp="H" then "H"
when r="R10" then "W"
when r="R02" then "B"
...
,case 
when age<=101 then age else 101 end as age102 length=3
,sex
,count(gq_id) as gq
from gq_next_3
group by r7,age102,sex;
quit;

proc sql noprint;
create table cest_3 as select x.*
,coalesce(y.gq,0) as gq
,x.p_cest - coalesce(y.gq,0) as hp_cest
from cest_1 as x
left join sandag_gq_2 as y
on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex;

select count(*) into :cest from cest_3 where hp_cest<0;
quit;

%macro cest;
%if &cest >0 %then %do;
%put !!!!! WARNING (negative hp_cest) !!!!! for year &yr;
%end;
%else %do;
%put ------------------- hp_cest is not negative for year &yr;
%end;
...
/* this is needed because the updated hp no longer matches the original */
proc sql;
create table dof_4 as select x.hp_dof,sum(y.hh_dof) as hh_dof
from (select sum(hp_cest) as hp_dof from hp_cest_1 /*cest_3*/) as x
cross join (select sum(hh_dof) as hh_dof from dof_3 where est_yr=&yrn) as y;
quit;


proc sql;
create table dof_5c as select r7,age102,sex,hp_cest as hp_dof
from hp_cest_1 /*cest_3*/;
quit;
...
group by r7,age102,sex,calculated size;

create table sandag_hh_1 as select r7,age102,sex,sum(hh) as hh from sandag_hh_0
group by r7,age102,sex;

create table sandag_hh_1s as select r7,age102,sex,size,sum(hh) as hh from sandag_hh_0
group by r7,age102,sex,size;
quit;

data size10;do size=1 to 10;length size 3;output;end;run;

proc sql;
create table test_hh_01 as select sex,age102,sum(hh) as hh from sandag_hh_0
where age102<=20 group by sex,age102;
quit;
...
left join sandag_hh_0 as y on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex and y.size=z.size
where x.age102>19 /* x.age18^="15_19" */
order by r7,age102,sex,size;

create table sandag_hh_2a as select * from sandag_hh_2 where age102>99;

/* removing households headed by persons 100 years and older */
update sandag_hh_2 set hh=0,hhr=0 where hh>0 and age102>99;
quit;

proc sql;
create table dof_6 as select x.r7,x.age102,x.sex,y.size,coalesce(y.hhr,0) as hhr
,round(x.hp_dof * coalesce(y.hhr,0),1) as hh0
from dof_5c as x
inner join sandag_hh_2 as y on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex
order by r7,age102,sex,size;

create table dof_7 as select x.r7,x.age102,x.sex,x.size,y.hh_dof/*,x.sum_hh0*/,x.hh0
,int(x.s * y.hh_dof) as hh1
from (select *,hh0/sum(hh0) as s,sum(hh0) as sum_hh0 from dof_6) as x
...
create table dof_9 as select x.r7,x.age102,x.sex,x.size
,case when x.i<=y.d then x.hh1+1 else x.hh1 end as hh2
,calculated hh2 * x.size as hp2
from dof_8 as x,dof_8a as y
order by r7,age102,sex,size;
...
from (select sum(hp2) as hp2 from dof_10) as x,dof_4 as y;
quit;
...
data dof_11(drop=hh2 i);set dof_9(drop=hp2);by r7 age102 sex size;do i=1 to hh2;output;end;run;
...
%if &a>=0 %then
	%do;
		proc sql;
			...
			create table dof_13 as select r7,age102,sex,case when i<=&a then size+1 else size end as size from dof_12a;
		quit;
	%end;
...
%else
	%do;
		proc sql;
			...
			create table dof_13 as select r7,age102,sex,case when i<=abs(&a) then size-1 else size end as size from dof_12a;
		quit;
	%end;

proc sql;
create table dof_14 as select x.r7,x.age102,x.sex,y.size,coalesce(z.hh,0) as hh
from (select distinct r7,age102,sex from dof_13) as x
cross join size10 as y
left join (select r7,age102,sex,size,count(*) as hh from dof_13 group by r7,age102,sex,size) as z
on x.r7=z.r7 and x.age102=z.age102 and x.sex=z.sex and y.size=z.size;
...
proc sql;
create table ztest_1 as select x.*,y.*
from (select sum(hp_dof) as hp1 from dof_5c) as x
cross join (select sum(size*hh) as hp2,sum(hh) as hh from dof_14) as y;

create table ztest_2 as select x.r7,x.age102,x.sex,x.hp_dof as hp,y.hh
from dof_5c as x
left join (select r7,age102,sex,sum(hh) as hh from dof_14 group by r7,age102,sex) as y
on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex;
...
from (select r7,age102,sex,count(hp_id) as hp from sandag_hp_1 group by r7,age102,sex) as x
left join (select r7,age102,sex,count(hh_id) as hh from sandag_hp_1 where role="H" group by r7,age102,sex) as y
on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex;
...
/*when calculated hp_d1<0 then abs(calculated hp_d1) + abs(calculated hh_d1)
when calculated hh_d1<0 then abs(calculated hh_d1) */
...
left join dof_5c as y on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex
left join (select r7,age102,sex,sum(hh) as hh_dof from dof_14 group by r7,age102,sex) as z
on x.r7=z.r7 and x.age102=z.age102 and x.sex=z.sex;
...
from sandag_hp_1 as x left join sandag_hp_2a as y on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex
order by r7,age102,sex,ranuni(&yr+1);
quit;

data sandag_hp_3;set sandag_hp_3;by r7 age102 sex;retain i;
...
from (select hh_id,hp_id,r,hisp,sex,age,dob,role,r7,age102 from hp_new_1 where role="H") as x
left join (select hh_id,count(hp_id) as size from hp_new_1 group by hh_id) as y
on x.hh_id=y.hh_id
order by r7,age102,size;

create table hh_new_1a as select r7,age102,sex,size,count(hh_id) as hh
from hh_new_1 group by r7,age102,sex,size;
...
from hh_new_1a as x left join dof_14 as y on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex and x.size=y.size;

create table hh_new_1c as select x.*,y.hh_d
from hh_new_1 as x left join hh_new_1b as y on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex and x.size=y.size
order by r7,age102,sex,size,ranuni(&yr+2);
quit;

data hh_new_1c;set hh_new_1c;by r7 age102 sex size;retain i;
...
from dof_5c as x
left join (select r7,age102,sex,sum(hh) as hh_dof from dof_14 group by r7,age102,sex) as y
	on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex
left join (select r7,age102,sex,count(hp_id) as hp from hp_new_2 group by r7,age102,sex) as z
	on x.r7=z.r7 and x.age102=z.age102 and x.sex=z.sex
left join (select r7,age102,sex,count(hh_id) as hh from hh_new_2 group by r7,age102,sex) as w
	on x.r7=w.r7 and x.age102=w.age102 and x.sex=w.sex;
...
from hp_new_2 as x
inner join sandag_hp_3b as y on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex
where x.role="H";

create table hp_list_1 as select x.*,z.d
from hp_new_2 as x
left join hh_list_1 as y on x.hh_id=y.hh_id
inner join sandag_hp_3b as z on x.r7=z.r7 and x.age102=z.age102 and x.sex=z.sex
where y.hh_id=.
order by r7,age102,sex,ranuni(&yr+3);
quit;

data hp_list_1;set hp_list_1;by r7 age102 sex;retain i;
if first.sex then i=1;else i=i+1;run;
...
where y.hh_id=.
order by r7,age102,sex,ranuni(&yr+4);

create table hh_new_3 as select x.*
from hh_new_2 as x left join hh_rem_3 as y on x.hh_id=y.hh_id
where y.hh_id=.
order by r7,age102,sex,ranuni(&yr+5);
quit;

data hp_new_3;set hp_new_3;by r7 age102 sex;retain i;
if first.sex then i=1; else i=i+1;
run;

data hp_rem_4;set hp_rem_1 hp_rem_2 hp_rem_3;rn=ranuni(&yr+6);run;
proc sort data=hp_rem_4;by r7 age102 sex rn;run;

data hp_rem_4;set hp_rem_4;by r7 age102 sex;retain i;
if first.sex then i=1; else i=i+1;
run;
...
left join (select r7,age102,sex,sum(hh) as hh_dof from dof_14 group by r7,age102,sex) as y
	on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex
left join (select r7,age102,sex,count(hp_id) as hp_new from hp_new_3 group by r7,age102,sex) as z
	on x.r7=z.r7 and x.age102=z.age102 and x.sex=z.sex
left join (select r7,age102,sex,count(hh_id) as hh_new from hp_new_3 where role="H" group by r7,age102,sex) as w
	on x.r7=w.r7 and x.age102=w.age102 and x.sex=w.sex
left join (select r7,age102,sex,count(hp_id) as hp_avl from hp_rem_4 group by r7,age102,sex) as u
	on x.r7=u.r7 and x.age102=u.age102 and x.sex=u.sex;
...
proc sql;
create table hp_clone_1 as select r7,age102,sex,hp_clone,hp_old as mx from dof_15 where hp_clone>0;
...
proc sql;
create table hp_clone_3 as select x.r7,x.age102,x.sex
,y.r,y.hisp,y.age,y.dob
from hp_clone_2 as x
left join hp_new_3 as y on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex and x.i=y.i;

/*
create table hp_clone_3_ as select x.r7,x.age102,x.sex
,y1.r as r_1,y1.hisp as hisp_1,y1.age as age_1,y1.dob as dob_1
,y2.r as r_2,y2.hisp as hisp_2,y2.age as age_2,y2.dob as dob_2
,y3.r as r_3,y3.hisp as hisp_3,y3.age as age_3,y3.dob as dob_3
from (select * from hp_clone_2 where mx=0) as x
left join hp_new_3 as y1 on x.r7=y1.r7 and x.age102=y1.age102+1 and x.sex=y1.sex and x.i=y1.i
left join hp_new_3 as y2 on x.r7=y2.r7 and x.age102=y2.age102+2 and x.sex=y2.sex and x.i=y2.i
left join hp_new_3 as y3 on x.r7=y3.r7 and x.age102=y3.age102+3 and x.sex=y3.sex and x.i=y3.i;
*/

create table hp_clone_3a as select * from hp_clone_3 where age=.;

update hp_clone_3 set age=age102,r="R02",hisp="NH",dob=mdy(12,10,&yr-age102) where age=. and r7="B";
update hp_clone_3 set age=age102,r="R03",hisp="NH",dob=mdy(12,11,&yr-age102) where age=. and r7="I";
update hp_clone_3 set age=age102,r="R07",hisp="NH",dob=mdy(12,12,&yr-age102) where age=. and r7="M";
update hp_clone_3 set age=age102,r="R05",hisp="NH",dob=mdy(12,13,&yr-age102) where age=. and r7="P";
update hp_clone_3 set age=age102,r="R04",hisp="NH",dob=mdy(12,14,&yr-age102) where age=. and r7="S";
update hp_clone_3 set age=age102,r="R10",hisp="NH",dob=mdy(12,15,&yr-age102) where age=. and r7="W";
update hp_clone_3 set age=age102,r="R11",hisp="H",dob=mdy(12,16,&yr-age102) where age=. and r7="H";
...
on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex and x.i<=y.use_avl;

create table hp_pool_0 as 
select * from hp_clone_3
	union all
select r7,age102,sex,r,hisp,age,dob from hp_avl_0;

create table hp_pool_0_ as select r7,age102,sex,r,hisp,age length=3,dob length=4
from hp_pool_0;

create table hp_pool_0 as select * from hp_pool_0_
order by r7,age102,sex,ranuni(&yr+7);

drop table hp_pool_0_;
quit;


data hp_pool_0;set hp_pool_0;by r7 age102 sex;length pid 5;retain i;
pid+1;
if first.sex then i=1;else i=i+1;
run;
...
from dof_14 as x
left join (select r7,age102,sex,size,count(hh_id) as hh_old from hh_new_3 group by r7,age102,sex,size) as y
on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex and x.size=y.size;
...
data dof_14c;set dof_14b;by r7 age102 sex size;retain a b;where hh_new>0;
...
from (select r7,age102,sex,sum(hh_new) as hh from dof_14c group by r7,age102,sex) as x
left join (select r7,age102,sex,count(pid) as pid from hp_pool_0 group by r7,age102,sex) as y
on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex;

/* this table should have zero records */
create table ztest_02 as select * from ztest_01 where hh>pid;

create table hp_pool_1 as select x.r7,x.age102,x.r,x.hisp,x.sex,x.age,x.dob,x.pid
,case when y.size^=. then "H" else "M" end as role
,y.size
,case
when x.age102<=4 then "00_04"
when x.age102<=9 then "05_09"
when x.age102<=14 then "10_14"
when x.age102<=19 then "15_19"
when x.age102<=24 then "20_24"
when x.age102<=29 then "25_29"
when x.age102<=34 then "30_34"
when x.age102<=39 then "35_39"
when x.age102<=44 then "40_44"
when x.age102<=49 then "45_49"
when x.age102<=54 then "50_54"
when x.age102<=59 then "55_59"
when x.age102<=64 then "60_64"
when x.age102<=69 then "65_69"
when x.age102<=74 then "70_74"
when x.age102<=79 then "75_79"
when x.age102<=84 then "80_84" else "85_99" end as age18 length=5
from hp_pool_0 as x left join dof_14c as y on x.r7=y.r7 and x.age102=y.age102 and x.sex=y.sex and y.a<=x.i<=y.b
order by pid;
...
from dof_5c as x
left join (select r7,age102,sum(hh) as hh_dof from dof_14 group by r7,age102) as y on x.r7=y.r7 and x.age102=y.age102
left join (select r7,age102,count(hp_id) as hp_new from hp_new_3 group by r7,age102) as z on x.r7=z.r7 and x.age102=z.age102
left join (select r7,age102,count(hh_id) as hh_new from hp_new_3 where role="H" group by r7,age102) as w on x.r7=w.r7 and x.age102=w.age102
left join (select r7,age102,count(*) as hp_cloned from hp_pool_1 group by r7,age102) as u on x.r7=u.r7 and x.age102=u.age102
left join (select r7,age102,count(*) as hh_cloned from hp_pool_1 where role="H" group by r7,age102) as v on x.r7=v.r7 and x.age102=v.age102;
quit;
...
/*
when age<=24 then "00_24"
when age<=34 then "25_34"
when age<=44 then "35_44"
when age<=54 then "45_54"
when age<=64 then "55_64"
when age<=74 then "65_74"
when age>=75 then "75_99" end as age7
*/
...
when x.r="R04" then "S"
when x.r="R05" then "P"
when x.r="R06" then "M" /* some other becomes M */
when x.r="R07" then "M" /* two or more becomes M */
end as r7 length=1
,case 
...
from config_4_rsa as x inner join config_id_1 as y on 
x.C00_09 = y.C00_09
...
/*
create table config_7_sa as select hh_age18,hh_sex,size,c_id,p/sum(p) as p
from (select x.hh_age18,x.hh_sex,x.size,x.c_id,x.p from config_6_sa as x inner join
(select distinct c_id from config_id_3) as y on x.c_id=y.c_id)
group by hh_age18,hh_sex,size;

create table config_7_rsa as select hh_r7,hh_age18,hh_sex,size,c_id,p/sum(p) as p
from (select x.hh_r7,x.hh_age18,x.hh_sex,x.size,x.c_id,x.p from config_6_rsa as x inner join
(select distinct c_id from config_id_3) as y on x.c_id=y.c_id)
group by hh_r7,hh_age18,hh_sex,size;
quit;
*/
...
/*	%put Failure to find a configuration;
	%goto exit1; */
%end;
...
/* proc sort data=hh_pool_6a;by hid pid;run; */
...
,x.r,x.hisp,x.sex,x.age,x.dob length=4,x.role
...
data hp_next_4b;set hp_next_4a hp_add_2;run;
...
create table hp_rem_5 as select * from hp_rem_4(drop=rn i r7 age102);
quit;

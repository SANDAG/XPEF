...
proc sql;
create table bld_location_0a as select * from bld_location_0 where mgra=. or jur_2017 = . or mgra=0;
create table prc_location_0a as select * from prc_location_0 where mgra=. or jur_2017 = . or mgra=0;
quit;


proc sql;
create table test_01 as select x.*
from (select distinct parcel_id from prc_location_0a) as x
left join (select distinct parcel_id from prc_location_0 where mgra>0) as y
on x.parcel_id=y.parcel_id
where y.parcel_id=.;
quit;
...
create table bld_location_2a as select building_id,count(*) as n
from bld_location_2 group by building_id having calculated n>1;
quit;


proc sql;
create table prc_location_2 as select parcel_id,mgra,jur_id,cpa_id,sum(js) as js,sum(j) as j
from bld_location_2 group by parcel_id,mgra,jur_id,cpa_id
order by parcel_id,js desc;

create table prc_location_2a as select parcel_id,count(*) as n
from prc_location_2 group by parcel_id having calculated n>1;

create table prc_location_2b as select x.*
from prc_location_2 as x
inner join prc_location_2a as y on x.parcel_id=y.parcel_id
order by parcel_id;
quit;
...
proc sql;
create table sej_test as select count(job_id) as sej
from jb_0_ where j_type="SE";
quit;
...
,coalesce(x.j,0) + coalesce(y.j,0) as j
,coalesce(x.j_ws,0)  + coalesce(y.wsj,0) as j_wsj
,coalesce(x.j_se,0)  + coalesce(y.sej,0) as j_sej
from jb_2 as x
...
select x.yr,x.mgra,x.jur_id,x.cpa_id,y.employment_type_id,x.j_wsj as j 
from (select * from usj_3 where j_wsj>0) as x
left join emp_type_id as y on x.sector_id = y.sandag_industry_id

	union all

select yr,mgra,x.jur_id,x.cpa_id,15 as employment_type_id,x.j_sej as j 
from (select * from usj_3 where j_sej>0) as x
left join emp_type_id as y on x.sector_id = y.sandag_industry_id

	union all

/* adding base year ws jobs */
select &by1-1 as yr,x.mgra,x.jur_id,x.cpa_id,y.employment_type_id,x.j_ws as j 
from (select * from jb_1 where j_ws>0) as x
left join emp_type_id as y on x.sector_id = y.sandag_industry_id

	union all

/* adding base year se jobs */
select &by1-1 as yr,mgra,x.jur_id,x.cpa_id,15 as employment_type_id,x.j_se as j 
from (select * from jb_1 where j_se>0) as x
left join emp_type_id as y on x.sector_id = y.sandag_industry_id

	union all

/* adding private households jobs */
select yr-1 as yr,mgra,jur_id,cpa_id,15 as employment_type_id,emp_pvt_hh as j
from privhj_3;
...
create table jobs_out_1a as select max(jobs) as max_jobs
from jobs_out_1;
...
create table sql_xpef.dw_jobs(bulkload=yes bl_options=TABLOCK) as select * from jobs_out_2;
quit;
...
select yr,mgra
,case 
when sector_id in (1,2) then 91
when sector_id in (9,10,11,12,13,14) then 92
when sector_id in (24,26) then 93
when sector_id in (23,25) then 94
when sector_id in (22,27) then 95
else sector_id end as sector
,sum(j) as j
from usj_3 group by yr,mgra,sector

	union all

/* base year jobs */
select &by1-1 as yr,mgra
,case 
when sector_id in (1,2) then 91
when sector_id in (9,10,11,12,13,14) then 92
when sector_id in (24,26) then 93
...
,sum(j) as j
from jb_1 group by mgra,sector;
quit;

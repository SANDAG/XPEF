...
from (select a.*, b.shape from [urbansim].[urbansim].[scs_scheduled_development] a
inner join urbansim.urbansim.parcel b on a.parcel_id = b.parcel_id) as x
...
where x.capacity_3 > 0 and x.scenario_id = &scsver);

create table cap_0 as select *
from connection to odbc
(SELECT 
	s.[scenario_id] as version_id
	,p.jur_id
	,CASE WHEN cpa_id = 0 then jur_id
		else cpa_id end as geo_id
	,s.[parcel_id]
	,x.cap_jurisdiction_id
	,s.scs_site_id
	,s.[scenario_cap] as capacity
FROM [urbansim].[urbansim].[scs_scenario_parcels] s
inner join [isam].[xpef04].[parcel2015_mgra_jur_cpa] p
	on s.parcel_id = p.parcel_id
inner join (select * from [urbansim].[urbansim].[scs_parcel] where scenario_id = &scsver) x
	on x.parcel_id = s.parcel_id
where s.scenario_id = &scsver
	and p.i = 1);
...
FULL JOIN (SELECT * FROM urbansim.urbansim.scs_parcel where scenario_id = 2 and scs_site_id is not null) b
...
FULL JOIN (SELECT * FROM urbansim.urbansim.scs_parcel where scenario_id = 2) b
	on a.parcel_id = b.parcel_id
) c
);
...
proc sql;
create table cap_0a as select
	*
	,case when jur_id ^= cap_jurisdiction_id then cap_jurisdiction_id
		else jur_id end as jur_id2
	,case when parcel_id in (185765,5308189) then 1909
		when jur_id ^= cap_jurisdiction_id then cap_jurisdiction_id
		else geo_id end as geo_id2
from cap_0;

create table cap_0b as select
	version_id,jur_id2 as jur_id,geo_id2 as geo_id,parcel_id,capacity
from cap_0a
where scs_site_id is null;
quit;
...
create table phase_update_0 as select
	...
	,case when c.scs_site_id is not null then 'sch'
...
create table phase_update_4 as select *
	,case when scs_site_id < 2035 and scs_site_id > 2020 then scs_site_id
...
create table phase_update_5a as select
	scs_site_id, min(phase_yr2) as phase_yr2_site
from phase_update_5
where scs_site_id is not null
group by scs_site_id;
...
left join phase_update_5a y
	on x.scs_site_id = y.scs_site_id;
...
from phase_update_6
group by geo_id2, phase_yr_final
order by geo_id2, phase_yr_final;
...
create table sdy_0a as select
	x.parcel_id, y.phase_yr2_site as phase_yr, x.scs_site_id
from sdy_0 x
left join phase_update_5a y
	on x.scs_site_id = y.scs_site_id;
quit;
...
create table cpa_target_1g as select *
	,case when (yr=2034 and geo_id = 2) then hu3 + round(rem_2034/3,1)
		when (yr=2034 and geo_id = 1911) then hu3 + round(2*(rem_2034/3),1)
		else hu3 end as hu4
from cpa_target_1f
order by yr, geo_id;
...
proc sql;
create table test_2 as select yr,sum(hu4) as c
...
,"" as max_units
,"SCS2 RHNA 2021-2034 v2" as scenario_desc

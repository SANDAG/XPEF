...
SELECT 
	...
	,[cap_priority]
	,subtier
	,CASE WHEN capacity_3 is not null THEN 1
		WHEN (subtier NOT LIKE 'SG%' AND (subtier LIKE '%1a' OR subtier LIKE '%1c') AND capacity_3 IS NULL) THEN 2
		WHEN (subtier NOT LIKE 'SG%' AND subtier LIKE '%1b' AND capacity_3 IS NULL) THEN 3
		WHEN (subtier = 'SG1a' AND capacity_3 IS NULL) THEN 4
		WHEN (subtier NOT LIKE 'SG%' AND subtier LIKE '%2' AND capacity_3 IS NULL) THEN 5
		WHEN (subtier = 'SG1b' AND capacity_3 IS NULL) THEN 6
		WHEN (subtier = 'SG2' AND capacity_3 IS NULL) THEN 7
		ELSE 8 END as subtier2
...
where ((target_priority1_cap + target_priority2_cap) > (parcel_priority1_cap + parcel_priority2_cap))  /* Scen 2 test */
/* where (target_priority1_cap ^= parcel_priority1_cap) OR (target_priority2_cap ^= parcel_priority2_cap) */ /* Scen 1 test */
order by a.jur_id;
...
create table ranking_0 as select c.id, a.*, (b.scs_priority1_cap + b.scs_distributed_tier2_cap) as at
from scs_parcel_0 a
left join scs_target_0 b
on a.cap_jurisdiction_id = b.jur_id and a.mohub2 = b.mohub2
left join jur_mohub_ids_1 c
on a.cap_jurisdiction_id = c.cap_jurisdiction_id and a.mohub2 = c.mohub2
order by id, subtier2, floor(score) desc, ranuni(&by1);

create table ranking_1 as select *
from ranking_0
where at > 0;

quit;

data ranking_2;set ranking_1;by id;retain tc;
if first.id then do;c1=min(scenario_cap,at);tc=c1;end;
else do;c1=min(scenario_cap,(at-tc));tc=tc+c1;end;
run;
...
create table ranking_3 as select
parcel_id,scs_site_id,mgra,cap_jurisdiction_id,mohub,score,scenario_cap,cap_priority,subtier2,
c1 as target_cap
from ranking_2
where c1 > 0;
...
create table z_test3 as select a.cap_jurisdiction_id, sum(b.scenario_cap) as sc from ranking_3 as a 
inner join scs_scenario_parcels b on a.parcel_id = b.parcel_id group by a.cap_jurisdiction_id;
create table z_test4 as select a.mohub, sum(b.scenario_cap) as sc from ranking_3 as a 
inner join scs_scenario_parcels b on a.parcel_id = b.parcel_id group by a.mohub;
create table z_test5 as select a.subtier2, sum(b.scenario_cap) as sc 
	from ranking_3 as a inner join scs_scenario_parcels as b on a.parcel_id = b.parcel_id group by subtier2;
